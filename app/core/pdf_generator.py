...
def generate_pdf_customer_details(customer_data):
    """Generate a PDF with customer details, payment summary, and payment history"""
    # Extract customer details
    customer_id = customer_data.get('id', '')
    customer_name = customer_data.get('customer_name', '')
    customer_code = customer_data.get('customer_code', '')
    customer_address = customer_data.get('address', '')
    customer_phone = customer_data.get('phone_no', '')
    ...
    payment_status = customer_data.get('payment_status', '')
    ...
    try:
        ...
        # Set title
        c.setTitle(f"Customer Details - {customer_name}")

        # Define colors
        header_color = colors.Color(0.2, 0.4, 0.8)  # Blue for headers
        light_gray = colors.Color(0.9, 0.9, 0.9)    # Light gray for backgrounds
        orange_color = colors.Color(1.0, 0.5, 0.0)  # Orange for partially paid
        ...
        logo_path = "static/images/logo.png"
        if os.path.exists(logo_path):
            c.drawImage(logo_path, 50, height - 85, width=80, height=60, mask='auto')

        # Add company name and header
        c.setFont("Helvetica-Bold", 18)
        c.setFillColor(header_color)
        c.drawString(200, height - 70, "CUSTOMER DETAILS")

        # Add date
        c.setFont("Helvetica", 10)
        c.drawString(width - 150, height - 60, f"Date: {datetime.now().strftime('%d-%m-%Y')}")

        # Add Sunmax Renewables text
        c.setFont("Helvetica-Bold", 12)
        c.setFillColor(colors.black)
        c.drawString(50, height - 110, "Sunmax Renewables")
        ...
        c.setFont("Helvetica-Bold", 12)
        c.setFillColor(header_color)
        c.drawString(50, y_position, "Company Information")

        # Add company details section
        y_position -= 20
        c.setFillColor(light_gray)
        c.rect(50, y_position - 100, (width - 100) / 2 - 10, 100, fill=1, stroke=0)

        # Add company details
        c.setFillColor(colors.black)
        c.setFont("Helvetica-Bold", 10)
        c.drawString(60, y_position - 20, "Company Name:")
        c.setFont("Helvetica", 10)
        c.drawString(160, y_position - 20, "Sunmax Renewables")

        c.setFont("Helvetica-Bold", 10)
        c.drawString(60, y_position - 40, "Address:")
        c.setFont("Helvetica", 9)
        c.drawString(160, y_position - 40, "Ground floor of Sneha Nagar Sachivalayam")
        c.drawString(160, y_position - 55, "B.V.Nagar, NGO Colony, Nellore")
        c.drawString(160, y_position - 70, "Andhra Pradesh 524004")

        c.setFont("Helvetica-Bold", 10)
        c.drawString(60, y_position - 90, "Phone:")
        c.setFont("Helvetica", 10)
        c.drawString(160, y_position - 90, "9381488225, 7013023946")

        c.setFont("Helvetica-Bold", 10)
        c.drawString(60, y_position - 110, "GST Number:")
        c.setFont("Helvetica", 10)
        c.drawString(160, y_position - 110, "37FUWPS9742A1ZT")

        # Add customer details section
        customer_section_x = (width - 100) / 2 + 50
        customer_section_y = y_position

        c.setFillColor(header_color)
        c.setFont("Helvetica-Bold", 12)
        c.drawString(customer_section_x, customer_section_y, "Customer Details")

        c.setFillColor(light_gray)
        c.rect(customer_section_x, customer_section_y - 100, (width - 100) / 2, 100, fill=1, stroke=0)

        c.setFillColor(colors.black)
        c.setFont("Helvetica-Bold", 10)
        c.drawString(customer_section_x + 10, customer_section_y - 20, "Customer Name:")
        c.setFont("Helvetica", 10)
        c.drawString(customer_section_x + 110, customer_section_y - 20, customer_name)

        c.setFont("Helvetica-Bold", 10)
        c.drawString(customer_section_x + 10, customer_section_y - 40, "Customer Code:")
        c.setFont("Helvetica", 10)
        c.drawString(customer_section_x + 110, customer_section_y - 40, customer_code)

        c.setFont("Helvetica-Bold", 10)
        c.drawString(customer_section_x + 10, customer_section_y - 60, "Phone Number:")
        c.setFont("Helvetica", 10)
        c.drawString(customer_section_x + 110, customer_section_y - 60, customer_phone)

        c.setFont("Helvetica-Bold", 10)
        c.drawString(customer_section_x + 10, customer_section_y - 80, "Address:")
        c.setFont("Helvetica", 9)
        if customer_address:
            # Handle multi-line address
            address_parts = customer_address.split('\n')
            for i, part in enumerate(address_parts[:2]):  # Limit to 2 lines
                c.drawString(customer_section_x + 110, customer_section_y - 80 - (i * 15), part)
        else:
            c.drawString(customer_section_x + 110, customer_section_y - 80, "")

        # Add product description section
        y_position = y_position - 130
        c.setFont("Helvetica-Bold", 12)
        c.setFillColor(header_color)
        c.drawString(50, y_position, "Product/Service Description")

        c.setFillColor(light_gray)
        c.rect(50, y_position - 60, width - 100, 40, fill=1, stroke=0)

        c.setFillColor(colors.black)
        c.setFont("Helvetica", 10)

        # Handle multi-line product description
        if product_description:
            desc_parts = product_description.split('\n')
            for i, part in enumerate(desc_parts[:2]):  # Limit to 2 lines
                c.drawString(60, y_position - 20 - (i * 15), part)
        else:
            c.drawString(60, y_position - 20, "No product description available.")

        # Add payment history header
        y_position = y_position - 80
        c.setFont("Helvetica-Bold", 12)
        c.setFillColor(header_color)
        c.drawString(50, y_position, "Payment History")

        # Add payment history table header
        y_position -= 20
        c.setFillColor(header_color)
        c.rect(50, y_position - 15, width - 100, 20, fill=1, stroke=0)

        # Add table headers
        c.setFillColor(colors.white)
        c.setFont("Helvetica-Bold", 10)
        c.drawString(75, y_position - 10, "Date")
        c.drawString(205, y_position - 10, "Amount")
        c.drawString(325, y_position - 10, "Method")
        c.drawString(445, y_position - 10, "Notes")
        ...
        if not payments:
            y_position -= 20
            c.drawString(75, y_position, "No payment history available")
        else:
            # Process payments
            for i, payment in enumerate(payments[:6]):  # Limit to 6 payments
                ...
                try:
                    ...
                    # Add payment details
                    y_position -= 15
                    c.drawString(75, y_position, date_str)
                    c.drawString(205, y_position, f"Rs.{float(amount):.2f}")
                    c.drawString(325, y_position, str(method))

                    # Truncate notes if too long
                    notes_str = str(notes)
                    if len(notes_str) > 20:
                        notes_str = notes_str[:17] + "..."
                    c.drawString(445, y_position, notes_str)

                    y_position -= 10
                except Exception as payment_error:
                    print(f"Error processing payment: {payment_error}")
                    y_position -= 15
                    c.drawString(75, y_position, f"Error processing payment: {str(payment_error)[:30]}")
                    y_position -= 10
        ...
        c.setFillColor(light_gray)
        c.rect(50, y_position - 100, width - 100, 80, fill=1, stroke=0)
        c.setStrokeColor(colors.black)
        c.rect(50, y_position - 100, width - 100, 80, fill=0, stroke=1)

        # Add payment summary details
        c.setFillColor(colors.black)
        c.setFont("Helvetica-Bold", 10)
        c.drawString(60, y_position - 25, "Total Amount:")
        c.drawString(60, y_position - 45, "Amount Paid:")
        c.drawString(60, y_position - 65, "Balance Due:")
        c.drawString(60, y_position - 85, "Payment Status:")

        # Add values
        c.setFont("Helvetica", 10)
        c.drawString(160, y_position - 25, f"Rs.{total_amount:.2f}")
        c.drawString(160, y_position - 45, f"Rs.{amount_paid:.2f}")
        c.drawString(160, y_position - 65, f"Rs.{(total_amount - amount_paid):.2f}")
        ...
        c.setFont("Helvetica-Bold", 10)
        c.drawString(160, y_position - 85, payment_status)
        c.setFillColor(colors.black)
        ...
        c.drawString(50, footer_y, f"Generated on: {datetime.now().strftime('%d-%m-%Y %H:%M:%S')}")
        ...
        error_details = traceback.format_exc()
        print(f"Error generating customer details PDF: {e}")
        print(f"Error details: {error_details}")
        ...
        try:
            ...
            # Set title
            c.setTitle(f"Customer Details - {customer_name} - Error")

            # Add error information
            c.setFont("Helvetica-Bold", 14)
            c.drawString(50, height - 100, "Error Generating Customer Details PDF")

            c.setFont("Helvetica", 12)
            c.drawString(50, height - 150, f"Customer: {customer_name}")
            c.drawString(50, height - 180, f"Error: {str(e)}")
