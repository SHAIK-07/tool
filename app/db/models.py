...
class Expense(Base):
    ...
    date = Column(Date, nullable=False, default=datetime.now().date())
    expense_type = Column(String, nullable=False)  # "Product Purchase", "Salary", "Utilities", "Office Supplies", etc.
    ...
    total_amount = Column(Float, nullable=False)
    amount_paid = Column(Float, default=0.0)  # Amount paid so far
    payment_method = Column(String, nullable=False)  # "Cash", "Bank Transfer", "Cheque", "UPI", etc.
    payment_status = Column(String, default="Paid")  # "Paid", "Pending", "Partially Paid"
    category = Column(String)  # "Operational", "Capital", "Administrative", etc.
    receipt_path = Column(String)  # Path to receipt/invoice file
    notes = Column(Text)
    created_at = Column(DateTime, default=datetime.now)
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now)

    # Relationship with expense payments
