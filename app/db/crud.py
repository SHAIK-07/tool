...
def generate_expense_code(db: Session) -> str:
    """Generate a unique expense code with format EXP001, EXP002, etc."""
    try:
        # Get the last expense by expense_code in descending order
        last_expense = db.query(models.Expense).order_by(
            models.Expense.expense_code.desc()
        ).first()

        if last_expense and last_expense.expense_code.startswith("EXP"):
            # Extract the numeric part after "EXP"
            numeric_part = ''.join(filter(str.isdigit, last_expense.expense_code))
            if numeric_part:
                number = int(numeric_part) + 1
            else:
                number = 1
        ...
        # Use 3 digits (zfill(3)) to allow for up to 999 expenses
        return f"EXP{str(number).zfill(3)}"
    except Exception as e:
        print(f"Error generating expense code: {e}")
        # Fallback to a timestamp-based code if there's an error
        import time
        timestamp = int(time.time()) % 10000  # Last 4 digits of timestamp
...
def create_expense(db: Session, expense_data: dict):
    ...
    try:
        ...
        if "expense_code" not in expense_data:
            expense_data["expense_code"] = generate_expense_code(db)

        # Calculate GST amount and total if not provided
        amount = expense_data.get("amount", 0)
        gst_rate = expense_data.get("gst_rate", 0)

        if "gst_amount" not in expense_data:
            expense_data["gst_amount"] = round(amount * (gst_rate / 100), 2)

        if "total_amount" not in expense_data:
            expense_data["total_amount"] = round(amount + expense_data["gst_amount"], 2)

        # Set amount_paid based on payment_status
        payment_status = expense_data.get("payment_status", "Paid")
        total_amount = expense_data["total_amount"]

        if payment_status == "Paid":
            expense_data["amount_paid"] = total_amount
        elif payment_status == "Partially Paid":
            # If not specified, set to 0 for partial payments (will be updated when payments are added)
            expense_data["amount_paid"] = expense_data.get("amount_paid", 0)
        else:  # Pending
            expense_data["amount_paid"] = 0
        ...
        if expense.amount_paid > 0:
            ...
            add_expense_payment(db, expense.id, payment_data)
...
def get_expense_by_code(db: Session, expense_code: str):
    """Get a specific expense by expense code"""
    return db.query(models.Expense).filter(models.Expense.expense_code == expense_code).first()
...
def update_expense(db: Session, expense_id: int, expense_data: dict):
    ...
    try:
        ...
        # Recalculate GST and total if amount or GST rate is updated
        if "amount" in expense_data or "gst_rate" in expense_data:
            amount = expense_data.get("amount", expense.amount)
            gst_rate = expense_data.get("gst_rate", expense.gst_rate)
            expense_data["gst_amount"] = round(amount * (gst_rate / 100), 2)
            expense_data["total_amount"] = round(amount + expense_data["gst_amount"], 2)
...
def search_expenses(db: Session, query: str):
    """Search for expenses by expense code, vendor name, or description"""
    from sqlalchemy import or_

    # Convert query to lowercase for case-insensitive search
    search_term = f"%{query}%"

    # Search in expense_code, vendor_name, and description columns
    expenses = db.query(models.Expense).filter(
        or_(
            models.Expense.expense_code.ilike(search_term),
            models.Expense.vendor_name.ilike(search_term),
            models.Expense.description.ilike(search_term),
            models.Expense.expense_type.ilike(search_term)
        )
    ).all()
...
def get_expenses_by_type(db: Session, expense_type: str, skip: int = 0, limit: int = 100):
    """Get expenses filtered by type"""
    return db.query(models.Expense).filter(
        models.Expense.expense_type == expense_type
    ).order_by(models.Expense.date.desc()).offset(skip).limit(limit).all()


def get_expenses_by_category(db: Session, category: str, skip: int = 0, limit: int = 100):
...
def get_expenses_by_payment_status(db: Session, payment_status: str, skip: int = 0, limit: int = 100):
    """Get expenses filtered by payment status"""
    return db.query(models.Expense).filter(
        models.Expense.payment_status == payment_status
    ).order_by(models.Expense.date.desc()).offset(skip).limit(limit).all()
...
def get_expense_stats(db: Session):
    ...
    # Total amount spent
    total_amount = db.query(func.sum(models.Expense.total_amount)).scalar() or 0.0

    # Total GST paid
    total_gst = db.query(func.sum(models.Expense.gst_amount)).scalar() or 0.0
    ...
    top_vendors = db.query(
        models.Expense.vendor_name,
        func.sum(models.Expense.total_amount).label('total_amount')
    ).group_by(models.Expense.vendor_name).order_by(
        func.sum(models.Expense.total_amount).desc()
    ).limit(5).all()
    ...
    return {
        ...
        "expenses_by_type": [{"type": exp[0], "amount": round(exp[1], 2), "count": exp[2]} for exp in expenses_by_type],
        "expenses_by_category": [{"category": exp[0], "amount": round(exp[1], 2), "count": exp[2]} for exp in expenses_by_category],
        "top_vendors": [{"vendor": vendor[0], "amount": round(vendor[1], 2)} for vendor in top_vendors]
    }


def add_expense_payment(db: Session, expense_id: int, payment_data: dict):
    ...
    try:
        ...
        # Calculate total amount paid for this expense (including the new payment)
        total_paid = db.query(func.sum(models.ExpensePayment.amount)).filter(
        ...
        # Update expense's amount_paid
        expense.amount_paid = total_paid

        # Update payment status based on amount paid
        if total_paid >= expense.total_amount:
            expense.payment_status = "Paid"
            expense.amount_paid = expense.total_amount  # Cap at total amount
        elif total_paid > 0:
        ...
        db.rollback()
        print(f"Error adding payment to expense {expense_id}: {e}")
        raise


def get_expense_payments(db: Session, expense_id: int):
    """Get all payments for a specific expense"""
    return db.query(models.ExpensePayment).filter(
        models.ExpensePayment.expense_id == expense_id
    ).order_by(models.ExpensePayment.payment_date.desc()).all()


def get_expense_with_payments(db: Session, expense_id: int):
    """Get an expense with its payment history"""
    expense = get_expense(db, expense_id)
    if not expense:
        return None

    # Get payments for this expense
    payments = get_expense_payments(db, expense_id)

    # Use the amount_paid from the database (which should be accurate)
    # But also verify it matches the sum of payments
    total_paid_from_payments = sum(payment.amount for payment in payments)

    # If there's a discrepancy, update the database
    if abs(expense.amount_paid - total_paid_from_payments) > 0.01:  # Allow for small rounding differences
        expense.amount_paid = total_paid_from_payments
        expense.updated_at = datetime.now()
        db.commit()
        db.refresh(expense)

    # Add calculated fields to expense object
    expense.remaining_amount = expense.total_amount - expense.amount_paid
